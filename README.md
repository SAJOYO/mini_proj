...
> (게임 화면 시연 도구에 현재 배율이 표시됩니다). 방치·경고색·여행은 배율을 올리는 것보다
> 아래 시연 도구의 `스탯 0 (방치)` · `여행 보내기 🧳`로 확인하는 쪽이 빠릅니다.

> **시연 도구는 기본으로 안 보입니다.** 실제 사용자가 볼 화면을 그대로 확인하려고
> 꺼둔 것입니다. 필요하면 `.env` 에 `EXPO_PUBLIC_DEMO_TOOLS=1` 을 넣고 개발 서버를
> 다시 띄우세요. 지울 때는 값을 비우면 됩니다. (APK 는 `.env` 가 안 먹으니
> [발표용 APK](#발표용-apk--시연-도구가-들어간-빌드) 참고.)
