...
> ⚠️ **폰의 Expo Go는 지금 안 됩니다.** 스토어에서 최신 버전을 받아도 안 됩니다.
>
> 이 프로젝트는 **SDK 57**인데 **App Store / Play 스토어의 Expo Go는 SDK 54까지만
> 지원**합니다. SDK 57용 Expo Go는 아직 스토어 심사 대기 중입니다.
> QR을 찍으면 "버전이 맞지 않는다"는 화면이 나옵니다 — 설정 문제가 아니라
> 앱이 아직 없는 것이라 팀원 누구도 폰에서 열 수 없습니다.
>
> 폰에서 확인하려면 SDK를 내리거나 개발 빌드(`eas build`)를 만들어야 합니다.
> **팀에서 정할 사안**입니다.

#### 웹에서 확인할 수 없는 것

웹은 `react-native-web`으로 도는 거라 네이티브와 다릅니다. 아래는 웹에서
"잘 되는 것처럼 보여도" 폰에서 확인해야 하는 항목입니다.
...
| `npm start`                       | 개발 서버 (QR 코드 — 현재 Expo Go 불가)      |
...
시연용으로 게임 화면 맨 아래에 `[개발용] 다음 단계로` 버튼이 있습니다 (`__DEV__`에서만 보임).
