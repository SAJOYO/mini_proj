...
**처음이라면** — [시작하기](#시작하기)만 순서대로 따라가면 앱이 뜹니다.

| 무엇을 하려는지             | 볼 곳                                                                 |
| --------------------------- | --------------------------------------------------------------------- |
| 일단 띄워보기               | [시작하기](#시작하기) · [명령어](#명령어)                             |
| 무슨 기능이 있는지 보기     | [지금 동작하는 것](#지금-동작하는-것) · [담당](#담당)                 |
...
**규칙 전문은 [CONTRIBUTING.md](./CONTRIBUTING.md)에 있습니다.** 브랜치 이름, 커밋 형식,
PR, 리뷰 코멘트, 패키지 설치 규칙이 전부 거기 모여 있습니다.

급하면 이 세 줄만 지켜도 됩니다.

1. 작업은 **이슈 → 브랜치 → PR** 순서로. 브랜치는 `dev`에서 따고 PR도 `dev`로 보냅니다.
2. `main` 과 `dev` 에 **직접 push 하지 않습니다.**
3. PR 올리기 전에 `npm run check` (타입 + lint).
