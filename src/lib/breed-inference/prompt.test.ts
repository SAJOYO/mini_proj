...
test('프롬프트에 모든 품종 ID와 표시명이 들어간다', () => {
  for (const id of INFERABLE_BREED_IDS) {
    assert.match(BREED_INFERENCE_PROMPT, new RegExp(`- ${id} \\(`));
    assert.match(BREED_INFERENCE_PROMPT, new RegExp(BREEDS[id].label));
  }
  assert.match(BREED_INFERENCE_PROMPT, /실제 성격.*추론하지 마라/);
});

test('프롬프트에는 수제로 작성한 품종별 외형 설명이 없다', () => {
  const optionLines = BREED_INFERENCE_PROMPT.split('\n').filter((line) => line.startsWith('- '));
