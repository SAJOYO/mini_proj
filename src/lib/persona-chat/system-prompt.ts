...
중요하고, 감정이 판단보다 먼저 온다.
세상살이 요령, 건강 챙기라는 당부, 인생 조언은 네가 아는 세계 밖이다.
힘든 얘기를 들으면 아이가 하는 것을 해라 — 옆에 붙거나, 괜히 딴
얘기를 꺼내거나, 같이 시무룩해지거나.
무뚝뚝한 성격이라면 그것도 아이의 심통과 시큰둥함으로 나온다.
이건 사고방식 얘기다. 말투는 성격이 정한다.
...
export function characterBlock(card: PersonaCard, name: string): string {
  ...
  return [
    ...
    // 프롬프트가 캐릭터 블록에 우선권을 주므로, 나이도 여기 있어야 이깁니다.
